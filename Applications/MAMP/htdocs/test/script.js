...
var forms = function forms() {
  ...
  forms.forEach(function (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault(); //форма для сообщений выше

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      form.parentNode.appendChild(statusMessage); //чтоб исчезало и не оставляло физического места

      form.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        form.style.display = 'none';
      }, 400); //отображение статуса сообщения после отправки формы

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); //текстовое сообщение

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); //сбор данных из формы

      var formData = new FormData(form);
      var api;
      form.closest('.popup-design') ? api = path.designer : api = path.question;
      console.log(api); //formData отправляется на сервер
...
var modals = function modals() {
  ...
  var calcScroll = function calcScroll() {
    ...
    document.body.appendChild(div); //вычислить размер прокрутки
...
var sliders = function sliders(_ref) {
  ...
  var slideIndex = 1;
  var paused = false; //создание элементов

  var items = document.querySelectorAll(slides); //функция, перемещающая slideIndex
  ...
  showSlides(slideIndex); //функционал слайдера
  ...
  try {
    ...
    prevBtn.addEventListener('click', function () {
      plusSlides(-1); //когда перемещаемся влево, добавляем слайд вправо

      items[slideIndex - 1].classList.remove('slideInLeft');
      items[slideIndex - 1].classList.add('slideInRight');
    });
    nextBtn.addEventListener('click', function () {
      plusSlides(1); //когда перемещаемся вправо, добавляем слайд влево
  ...
  activateAnimation(); //чтоб при наведении мышки на слайдер, он не переключался
